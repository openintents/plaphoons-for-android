...
public class SquareGridLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int spec = MeasureSpec.makeMeasureSpec(spp / d, MeasureSpec.EXACTLY);
		...
		setMeasuredDimension(
			mw == MeasureSpec.EXACTLY ? sw : (sp / d) * mRows + pw,
			mh == MeasureSpec.EXACTLY ? sh : (sp / d) * mColumns + ph
		);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int y = 0; y < mRows; y++) {
			for (int x = 0; x < mColumns; x++) {
				...
				child.layout(
					l + (s *  x   ) / d + lps.leftMargin,
					t + (s *  y   ) / d + lps.topMargin,
					l + (s * (x+1)) / d - lps.rightMargin,
					t + (s * (y+1)) / d - lps.bottomMargin
				);
			}
		}
	}
